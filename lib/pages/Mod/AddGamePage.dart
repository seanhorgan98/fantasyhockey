...
import 'package:fantasy_hockey/classes/AddGamePlayerPopup.dart';
...
class AddGamePageState extends State<AddGamePage> {
          ...
          children: <Widget>[
            ...
            TextField(
              decoration: InputDecoration(
                  border: InputBorder.none, hintText: 'Opponent Name'),
  ...
  void setPlayerName(BuildContext context) {
    _asyncSimpleDialogAdd(context).then((data) {
      if (data != null) {
        setState(() {
          players.add(data);
        });
  ...
  void subGame() async {
    ...
    //Conformation popup
    await _asyncConfirmPopup(context).then((int result) {
  ...
  */
  ...
  Future<void> _ackAlert(
      BuildContext context, String alertTitle, String alert) {
